...
function setAudioForSelectedSong() {
  ...
  if (!songs[artist] || !songs[artist][song]) {
    // Überprüfung ob Song gefunden wird -> nur vorhandene Songs werden geladen
...
function startAudio() {
  ...
  if (audio.readyState < 2) {
    // ausreichend Daten geladen -> klappt leider nicht (noch problematisch -> zeitlich keine Lösung gefunden -> beim zweiten Laden ist die Audio immer ready)
...
function handleKeyPress(key) {
  ...
  notes.forEach((note, index) => {
    if (note.key === key && Math.abs(note.y - NOTE_TARGET_Y) < HIT_THRESHOLD) {
      // wenn richtige Taste zum Zeitpunkt wo sich Note im Zielbereich befindet gedrückt wird
      hit = true;
...
function filterNotesByDifficulty(riff) {
  ...
  return riff.filter(
    (note) =>
      ...
      (difficulty === "medium" &&
        (note.difficulty === "easy" || note.difficulty === "medium")) || // wenn Schwierigkeit "medium" sollen alle Noten "easy" und "medium" geladen werden
      (difficulty === "hard" &&
        (note.difficulty === "easy" ||
          note.difficulty === "medium" ||
          note.difficulty === "hard"))
  ); // wenn Schwierigkeit "hard" sollen alle Noten geladen werden
}
